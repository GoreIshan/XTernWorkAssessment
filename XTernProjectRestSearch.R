...
recommendRestaurants <- function(maxTime = 200, cuisine = "all", minRating = 0, 
                                 maxCost = 200, maxDist = 1000, longitude = 0, latitude = 0){
  ...
  # filter by cost
  tweakedData <- tweakedData %>% filter(maxCost >= tweakedData$Average_Cost)
  # add the $ back
...
# maxTime specifies the maximum cooking time in minutes, cuisine specifies the desired cuisines,
# minRating specifies the minimum rating of the restaurant, maxCost specifies the maximum average cost,
# maxDist specifies the maximum distance (in miles) the restaurant should be, and 
# longitude and latitude specify the user's current longitude and latitude
recommendMe <- recommendRestaurants(maxTime = 40, cuisine = "Fast Food", minRating = 4,  maxCost = 20, 
...
data.frame(recommendMe$Restaurant)
